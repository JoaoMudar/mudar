...
| **Tarefa recorrente** | A rotina fixa que não se lança todo dia: um tipo de tarefa, um funcionário ou grupo, os dias da semana em que vale e a **hora de início e de fim**. É a única parte do planejamento que declara hora, e declara justamente por já tê-la na vida real. Gera atribuições na agenda; alterar o dia gerado não altera a regra. | "todo dia de manhã", "é fixo" | **Tarefa recorrente** (`task_recurrence`) |
