...
- **Não invente identificadores.** `RN-01` a `RN-70` (regras), `RF-01` a `RF-76` (requisitos
...
### 2.4 Três regras que são convenção, não fato observado

O critério da seção 2.1 (apague o sistema e veja se o enunciado sobrevive) é limpo para quase
todo o catálogo, e áspero para três regras da área C, todas nascidas ao desenhar a agenda de
pessoal:

| RN | Por que merece ressalva |
|---|---|
| **RN-48** | "Um turno vale quatro horas" é **convenção adotada**, não medição. O viveiro trabalha por turno de fato (isso é observação), mas o valor de quatro horas foi fixado para tornar o custo de mão de obra apurável sem controle de ponto |
...
| **RN-51** | Assumir o planejado como realizado é **escolha metodológica** diante de dado faltante. A alternativa (agenda com buraco) inviabiliza o custo do período; a condição fica registrada justamente para que a suposição não se disfarce de medição |
...
inexato: a origem correta das três é **ORG**, política do projeto, e é assim que `B2` as marca
(RF-73 e RF-75).
...
| **RN-60** | A pessoa que se **opõe ao contato comercial** fica fora de qualquer cotação ou abordagem, e essa oposição é registro permanente, não estado operacional que se desfaz sozinho | Restrição | `C6` §2; `C8` §`parties`; `E5` | RF-37 *(parcial)* | RNF-23 |
| **RN-61** | Declarar que **dois cadastros são a mesma pessoa é decisão de quem conhece as pessoas**: coincidência de nome ou de documento levanta a pergunta, não a conclui. A ambiguidade é aceita, não resolvida automaticamente | Restrição | `rotinas/4-financeiro/01-cadastro-unico.md` | RF-39 *(parcial)* | - |
...
| **RN-37** | A **carga** é gerada a partir dos itens aprovados e separada fisicamente em campo; é ela, e não o pedido, a unidade que sai do viveiro. Um pedido pode gerar mais de uma carga, e a entrega da última não fecha o ciclo por si só, ver RN-63 e RN-70 | Fato | `A2` §3; `C2` UC-27 | RF-46, RF-47, RF-50, RF-51 | RNF-01, RNF-05 |
...
| **RN-63** | **Cada carga é uma viagem, com data própria.** Cargas de um mesmo pedido saem em dias diferentes, e a data do pedido é referência, não a data de cada carga | Fato | `rotinas/3-comercial/00-visao-geral.md`; `pedidos/06-separacao-pedido.md` | RF-46, RF-50, RF-51 | - |
| **RN-64** | No item genérico, o recipiente informado é o **mínimo aceitável**: atende-se com porte igual ou maior, nunca menor | Restrição | `rotinas/3-comercial/pedidos/00-visao-geral.md`; `04-verificacao-disponibilidade.md` | RF-66, RF-67, RF-68 | - |
| **RN-65** | O **fornecedor que não responde deixa de ser consultado**: a confiabilidade se mede pela taxa de resposta, e a cotação deixa de ser disparo cego | Acionamento | `G2`; `C8` §`suppliers` | RF-52, RF-53 | - |
...
| RF-35 | RN-24, RN-58 |
| RF-36 | RN-26 |
| RF-37 | RN-25, RN-30, RN-60, RN-62 |
| RF-38 | RN-25 |
| RF-39 | RN-26, RN-27, RN-61 |
| RF-40 | RN-25, RN-28 |
| RF-41 | RN-04, RN-20, RN-31 |
| RF-42 | RN-06, RN-13, RN-34, RN-67 |
| RF-43 | RN-34, RN-67 |
| RF-44 | RN-21, RN-36, RN-59 |
| RF-45 | RN-25, RN-28, RN-29 |
| RF-46 | RN-37, RN-63, RN-66 |
| RF-47 | RN-37 |
| RF-48 | RN-31, RN-46 |
| RF-49 | RN-31 |
| RF-50 | RN-37, RN-63, RN-66 |
| RF-51 | RN-13, RN-31, RN-37, RN-63, RN-70 |
| RF-52 | RN-27, RN-38, RN-65 |
| RF-53 | RN-04, RN-38, RN-65 |
| RF-54 | RN-38, RN-39 |
| RF-55 | RN-23, RN-38 |
| RF-56 | RN-40, RN-70 |
...
|---|---|---|
| RNF-01 | RE-1: usuários sem formação técnica | RN-10, RN-16, RN-26, RN-37, RN-48, RN-49, RN-52, RN-55 |
...
| RNF-23 | **Regra de negócio** | **RN-30, RN-60** (Lei nº 13.709/2018 e oposição do titular) |
...
[`B5`](B5-matriz-rastreabilidade.md) §5 tratou as 25 que encontrou. Sete das regras acrescentadas
...
| **RN-60** | RF-37 | Registrar a oposição do titular e **excluí-lo das cotações** |
| **RN-61** | RF-39 | Apontar cadastros possivelmente duplicados e **submeter a união a decisão humana** |
...
catalogação. Quando forem especificadas, entram como **RF-77 em diante**: nenhum identificador se
reutiliza (seção 8).
