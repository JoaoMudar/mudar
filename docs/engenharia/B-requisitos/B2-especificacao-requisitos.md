...
| ID | Requisito | Ator | Prior. | Origem | Verificação |
|---|---|---|---|---|---|
| **RF-08** | O sistema deve permitir cadastrar espécie com nome científico, nomes populares, categoria, tempo de germinação, tempo de produção e fotografia | Gerência | D | EN, DOM | Espécie cadastrada aparece nas demais telas do sistema |
...
| **RF-10** | O sistema deve permitir cadastrar recipientes com volume e consumo de substrato por unidade | Gerência | D | EN | Recipiente cadastrado fica disponível para associação a espécies |
...
| **RF-37** | O sistema deve permitir cadastro completo de cliente com dados fiscais de pessoa física ou jurídica | Chefia, Gerência | D | LEG, AD | Cadastro completo comporta os dados exigidos para emissão de nota |
| **RF-38** | O sistema deve validar CPF e CNPJ informados | — | D | LEG | Documento inválido é recusado no momento da digitação |
| **RF-39** | O sistema deve permitir localizar cliente por nome, telefone ou documento | Chefia, Gerência | D | OP | Busca retorna o cliente por qualquer dos três |
...
### 2.10 Fornecedores e cotações

| ID | Requisito | Ator | Prior. | Origem | Verificação |
|---|---|---|---|---|---|
| **RF-52** | O sistema deve permitir cadastrar fornecedor com contato, localização e espécies que fornece | Gerência | DV | EN | Fornecedor cadastrado aparece na seleção de cotação |
| **RF-53** | O sistema deve permitir registrar cotação dirigida a um ou mais fornecedores, com espécie, recipiente e quantidade por item | Gerência | DV | EN | Cotação registrada permite lançar as respostas recebidas |
| **RF-54** | O sistema deve permitir comparar as propostas recebidas e registrar a escolhida por item | Chefia, Gerência | DV | EN | Comparação exibe as propostas lado a lado e registra a escolha |
| **RF-55** | O sistema poderia apresentar os fornecedores em mapa, com a distância até o viveiro | Gerência | P | EN | Fornecedor com endereço aparece posicionado no mapa |

### 2.11 Financeiro

> Subsistema de acesso restrito. Ver [`D4 — Matriz RBAC`](../D-arquitetura/D4-matriz-rbac.md).
...
| **RF-62** | O sistema deve restringir todo o subsistema financeiro aos perfis chefia e administrador | — | D | EN | Gerência e colaborador não acessam nenhuma tela financeira |
...
| **Precisão do custo × esforço de apuração** — apurar mão de obra por espécie exigiria apontamento de horas individual | Chefia × colaborador | Mão de obra entra por tempo médio estimado por atividade, e não por apontamento individual. Precisão suficiente para revelar margem negativa, que é o objetivo, sem impor controle de ponto por tarefa. |
