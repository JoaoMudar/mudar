...
- [ ] Criar migracao `migrations/20260521100001_pedidos_customers.sql`
- [ ] Campos:
...
- [ ] Index: `idx_customers_name` em `name`
- [ ] Index: `idx_customers_active` em `active`

### T1.2 — Tabela `orders` (pedidos)
- [ ] Criar migracao `migrations/20260521100002_pedidos_orders.sql`
- [ ] Campos:
  - `id` UUID DEFAULT gen_random_uuid() PRIMARY KEY
  ...
  - `created_at` TIMESTAMPTZ DEFAULT now()
  - `updated_at` TIMESTAMPTZ DEFAULT now()
- [ ] CHECK constraint em `status`: valores validos conforme lista de statuses
- [ ] Index: `idx_orders_status` em `status`
- [ ] Index: `idx_orders_customer_id` em `customer_id`
- [ ] Index: `idx_orders_delivery_date` em `delivery_date`
- [ ] Index: `idx_orders_created_at` em `created_at`

### T1.3 — Tabela `order_items` (itens do pedido)
- [ ] Criar migracao `migrations/20260521100003_pedidos_order_items.sql`
- [ ] Campos:
  - `id` UUID DEFAULT gen_random_uuid() PRIMARY KEY
  ...
  - `created_at` TIMESTAMPTZ DEFAULT now()
- [ ] Index: `idx_order_items_order_id` em `order_id`
- [ ] Index: `idx_order_items_parent_item_id` em `parent_item_id`
- [ ] CHECK constraint: se `is_generic = true` entao `species_id IS NULL`
- [ ] CHECK constraint: se `is_generic = false AND parent_item_id IS NULL` entao `species_id IS NOT NULL`
...
- [ ] Criar migracao `migrations/20260521100004_pedidos_order_loads.sql`
- [ ] Campos:
  - `id` UUID DEFAULT gen_random_uuid() PRIMARY KEY
  - `order_id` UUID NOT NULL REFERENCES orders(id) ON DELETE CASCADE
  ...
  - `created_at` TIMESTAMPTZ DEFAULT now()
  - `updated_at` TIMESTAMPTZ DEFAULT now()
- [ ] UNIQUE constraint em (order_id, load_number)
- [ ] Index: `idx_order_loads_order_id` em `order_id`

### T1.5 — Tabela `order_load_items` (itens por carga)
- [ ] Criar migracao `migrations/20260521100005_pedidos_order_load_items.sql`
- [ ] Campos:
  - `id` UUID DEFAULT gen_random_uuid() PRIMARY KEY
  ...
  - `created_at` TIMESTAMPTZ DEFAULT now()
- [ ] UNIQUE constraint em (load_id, order_item_id) — mesmo item nao aparece 2x na mesma carga
- [ ] Index: `idx_order_load_items_load_id` em `load_id`
...
- [ ] Criar migracao `migrations/20260521100006_pedidos_status_history.sql`
- [ ] Campos:
  - `id` UUID DEFAULT gen_random_uuid() PRIMARY KEY
  - `order_id` UUID NOT NULL REFERENCES orders(id) ON DELETE CASCADE
  ...
  - `notes` TEXT
  - `created_at` TIMESTAMPTZ DEFAULT now()
- [ ] Index: `idx_order_status_history_order_id` em `order_id`

### T1.7 — Tabela `notifications` (notificacoes in-app)
- [ ] Criar migracao `migrations/20260521100007_pedidos_notifications.sql`
- [ ] Campos:
  - `id` UUID DEFAULT gen_random_uuid() PRIMARY KEY
  ...
  - `created_at` TIMESTAMPTZ DEFAULT now()
- [ ] Index: `idx_notifications_user_id_read` em (user_id, read)
- [ ] Index: `idx_notifications_created_at` em `created_at`

### T1.8 — Rodar migracoes e validar
- [ ] Executar `npm run db:migrate` e confirmar que todas as tabelas foram criadas
- [ ] Verificar com query `SELECT table_name FROM information_schema.tables WHERE table_schema='public'`
